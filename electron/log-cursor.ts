...
const log = createLogger("log-cursor");
...
export function createLogCursor(userDataDir: string): LogCursor {
  ...
  function read(): CursorState {
    try {
      const parsed = JSON.parse(fs.readFileSync(file, "utf8")) as Partial<CursorState>;
      ...
      // No file yet (first run), or one we can't read. Either way we know nothing, and the
      // watcher's fallback — anchor at EOF — is the safe direction: we miss a gap rather than
      // replay a history.
      return { version: 1, files: {} };
    }
  }
  ...
  function write(): void {
    ...
    try {
      ...
      log.warn("could not save log positions:", (e as Error).message);
    }
  }
