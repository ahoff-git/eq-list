 ...
 */
test("a pack is named from its own labels, never a neighbouring pack's", () => {
  const { logDir, mapsDir } = tempMaps();
  ...
  const stock = namer.names(sources[0]);
  const brewall = namer.names(sources[1]);
...
test("naming one pack is not affected by another pack appearing beside it", () => {
  ...
  const solo = createZoneNamer().names(listSources(alone.logDir).sources[0]);
  ...
  const withPack = createZoneNamer().names(listSources(beside.logDir).sources[0]);
