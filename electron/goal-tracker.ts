...
function load(file: string): Stored {
  ...
  return { goals: stored.goals ?? [], templates: stored.templates ?? [] };
}
...
export function createGoalTracker({
  ...
  // `spawn-tracker.ts` applies to a timer found already overdue at launch.
  {
    const at = now();
    let marked = false;
    for (const goal of state.goals) {
      if (!goal.resultAnnounced && goalState(goal, at) !== "running") {
        goal.resultAnnounced = true;
        marked = true;
      }
    }
